...
function addUpcomingMeetups(chapter_ids) {
   var meetup_key = '54f2d2729675562237d3b1f6f3e3947';
...
function buildHtml(data) {
    ...
    $('#upcomingMeetupsList').append('<ul class=""></ul>');
    for (var key in data.results) {
      url_link = create_urls(data.results[key].event_url);
      event_date = convert_time_miliseconds_to_date(data.results[key].time);
      var html = '<img src="'+data.results[key].group.group_photo.thumb_link+'" height="80" width="80" />';
      html += '<p>'+data.results[key].group.name+'</p>';
      html += '<p><a href='+url_link+'>'+data.results[key].name+'</a></p>';
      html += '<p>'+event_date+'</p>';
